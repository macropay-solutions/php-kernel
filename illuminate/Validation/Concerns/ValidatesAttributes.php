...
trait ValidatesAttributes
{
    ...
    {
        try {
            return @\appDate()->parse($value) ?: null;
        } catch (Exception) {
            //
        }
    }
    ...
    /**
     * Validate that an array has all the given keys.
     *
     * @param string $attribute
     * @param mixed $value
     * @param array<int, int|string> $parameters
    ...
    public function validateBetween($attribute, $value, $parameters)
    {
        ...
        $toValidate = (string)$this->getSize($attribute, $value);
        $minThreshold = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($parameters[0]));
        $maxThreshold = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($parameters[1]));

        $precision = $this->getPrecision($toValidate, $minThreshold, $maxThreshold);

        return \bccomp($toValidate, $minThreshold, $precision) >= 0
            && \bccomp($toValidate, $maxThreshold, $precision) <= 0;
    }
    ...
    public function validateExists($attribute, $value, $parameters)
    {
        ...
        if (\count($parameters) > 2) {
            throw new \RuntimeException('Framework Security: Extended string syntax disabled for exists rule on [' .
                $attribute . ']. Extra where parameters must be provided using Rule::exists().');
        }
    ...
    public function validateUnique($attribute, $value, $parameters)
    {
        ...
        if (\count($parameters) > 2) {
            throw new \RuntimeException(
                'Framework Security: Extended string syntax or string update exclusions disabled for unique rule on [' .
                    $attribute . ']. Extra parameters or update exclusions must be provided using Rule::unique().'
            );
        }
    ...
    public function validateGt($attribute, $value, $parameters)
    {
        ...
        if (is_null($comparedToValue) && (is_numeric($value) && is_numeric($parameters[0]))) {
            $toValidate = (string)$this->getSize($attribute, $value);
            $threshold = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($parameters[0]));

            $precision = $this->getPrecision($toValidate, $threshold);

            return \bccomp($toValidate, $threshold, $precision) === 1;
        }
        ...
        if ($this->hasRule($attribute, $this->numericRules) && is_numeric($value) && is_numeric($comparedToValue)) {
            $toValidate = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($value));
            $threshold = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($comparedToValue));
    ...
    public function validateGte($attribute, $value, $parameters)
    {
        ...
        if (is_null($comparedToValue) && (is_numeric($value) && is_numeric($parameters[0]))) {
            ...
            return \bccomp($toValidate, $threshold, $precision) >= 0;
        }
    ...
    public function validateMultipleOf($attribute, $value, $parameters)
    {
        ...
        $numerator = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($value));
        $denominator = $this->ensureExponentWithinAllowedRange($attribute, $this->trim($parameters[0]));

        $precision = $this->getPrecision($numerator, $denominator);

        if (\bccomp($numerator, '0', $precision) === 0 && \bccomp($denominator, '0', $precision) === 0) {
            return false;
        }

        if (\bccomp($numerator, '0', $precision) === 0) {
            return true;
        }

        if (\bccomp($denominator, '0', $precision) === 0) {
            return false;
        }

        return \bccomp(\bcmod($numerator, $denominator, $precision), '0', $precision) === 0;
    }
    ...
    protected function ensureExponentWithinAllowedRange($attribute, $value)
    {
        ...
        if (!is_numeric($value) || false === \stripos($stringValue, 'e')) {
            return $value;
        }

        $parts = explode('e', strtolower($stringValue));
        $base = $parts[0];
        $exponent = (int)$parts[1];

        $withinRange = (
            $this->ensureExponentWithinAllowedRangeUsing ?? fn($scale) => $scale <= 1000 && $scale >= -1000
        )($exponent, (string)$attribute, $stringValue);
    ...
    /**
     * Determine the maximum decimal precision across multiple values.
     */
    protected function getPrecision(mixed ...$values): int
    {
        $maxDecimals = 0;

        foreach ($values as $value) {
            $stringValue = (string)$this->trim($value);
            $position = \strpos($stringValue, '.');

            if ($position !== false) {
                $maxDecimals = (int)\max($maxDecimals, \strlen($stringValue) - $position - 1);
            }
        }
