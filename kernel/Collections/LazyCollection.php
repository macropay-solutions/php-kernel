...
//        return new static(function () use ($from, $to) {
        return \di(static::class, [function () use ($from, $to) {
            if ($from <= $to) {
                for (; $from <= $to; $from++) {
                    yield $from;
                }
     ...
     * BEWARE THAT THIS CONVERTS THE LAZY COLLECTION INTO A COLLECTION AND LOADS ALL DATA IN MEMORY!!!
     ...
     * BEWARE THAT THIS CONVERTS THE LAZY COLLECTION INTO A COLLECTION AND LOADS ALL DATA IN MEMORY!!!
     * @param \MacropaySolutions\Kernel\Support\Enumerable<array-key, TKey>|array<array-key, TKey> $keys
     * @return static
     */
...
//                    $chunk = new Collection;
    ...
    public function zip($items)
    {
        ...
        return new static(function () use ($iterables) {
            ...
            while ($iterators->contains(static fn($iterator) => $iterator->valid())) {
                yield new static($iterators->map(static fn($iterator) => $iterator->current()));

                $iterators->each(static fn($iterator) => $iterator->next());
            }
        });
    }
