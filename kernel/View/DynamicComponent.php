...
use MacropaySolutions\Kernel\View\Compilers\ComponentTagCompiler;
...
class DynamicComponent extends Component
{
     ...
     *
     * @return \MacropaySolutions\Kernel\Contracts\View\View|string
     */
...
<?php extract(
    collect(
        ...
        fn($value, $key) => [MacropaySolutions\Kernel\Support\Str::camel(str_replace([':', '.'], ' ', $key)) => $value]
    )->all(),
    EXTR_SKIP
); ?>
{{ props }}
        ...
        return function ($data) use ($template) {
            ...
            return str_replace(
                ...
                [
                    ...
                    $this->compileSlots($data['__framework_slots']),
                    '{{ $slot ?? "" }}',
                ],
                $template
            );
