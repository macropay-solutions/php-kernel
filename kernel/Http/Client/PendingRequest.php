...
use MacropaySolutions\Kernel\Http\Client\Events\ConnectionFailed;
use MacropaySolutions\Kernel\Http\Client\Events\RequestSending;
use MacropaySolutions\Kernel\Http\Client\Events\ResponseReceived;
...
class PendingRequest
{
     ...
     *
     * @var \MacropaySolutions\Kernel\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \MacropaySolutions\Kernel\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param \MacropaySolutions\Kernel\Http\Client\Factory|null $factory
...
//        $this->middleware = new Collection($middleware);
        $this->middleware = \di(Collection::class, [$middleware]);
     ...
     * @return \MacropaySolutions\Kernel\Http\Client\Response
     */
     ...
     *
     * @param callable $callback
     * @return array<array-key, \MacropaySolutions\Kernel\Http\Client\Response>
     */
...
//                $this->dispatchConnectionFailedEvent(new Request($e->getRequest()));
                $this->dispatchConnectionFailedEvent(\di(Request::class, [$e->getRequest()]));
...
//                    $this->dispatchConnectionFailedEvent(new Request($e->getRequest()));
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $frameworkData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $frameworkData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = Str::of($url);

        if (empty($frameworkData) && $method === 'GET' && $urlString->contains('?')) {
            $frameworkData = (string)$urlString->after('?');
        }

        if (is_string($frameworkData)) {
            parse_str($frameworkData, $parsedData);

            $frameworkData = is_array($parsedData) ? $parsedData : [];
        }

        if ($frameworkData instanceof JsonSerializable) {
            $frameworkData = $frameworkData->jsonSerialize();
        }

        return is_array($frameworkData) ? $frameworkData : [];
    }
...
//                        (new Request($request))->withData($options['framework_data']),
                        \di(Request::class, [$request])->withData($options['framework_data']),
        ...
        return function ($handler) {
            return function ($request, $options) use ($handler) {
                $response = ($this->stubCallbacks ?? collect())
                    ->map
//                     ->__invoke((new Request($request))->withData($options['framework_data']), $options)
                    ->__invoke(\di(Request::class, [$request])->withData($options['framework_data']), $options)
                    ->filter()
                    ->first();
...
//                    $callback, (new Request($request))->withData($options['framework_data']), $options, $this
                    $callback,
                    \di(Request::class, [$request])->withData($options['framework_data']),
                    $options,
                    $this
...
//        return new Response($response);
        return \di(Response::class, [$response]);
    }
