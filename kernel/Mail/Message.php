...
class Message
{
    ...
    /**
     * Remove all the blind carbon copy addresses from the message.
     *
     * @return $this
     */
     ...
     * @param string|null $name
     */
    protected function createAddress($address, $name = null): Address
    {
        $this->ensureAddressIsSafe($address);
...
//                        (new DataPart(new File($path), $cid, $file->mime))->asInline()
                        (new DataPart(\di(File::class, [$path]), $cid, $file->mime))->asInline()
                    );
...
//            (new DataPart(new File($file), $cid))->asInline()
            (new DataPart(\di(File::class, [$file]), $cid))->asInline()
        );
