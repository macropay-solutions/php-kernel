...
use MacropaySolutions\Kernel\Contracts\Filesystem\LockTimeoutException;
use MacropaySolutions\Kernel\Filesystem\Filesystem;
use MacropaySolutions\Kernel\Filesystem\LockableFile;
...
class FileStore implements Store, LockProvider
{
    ...
    /**
     * The MacropaySolutions Kernel Filesystem instance.
     *
    ...
    /**
     * Refresh the lock if it is owned by the given owner.
     */
    public function refreshIfOwned(string $name, string $owner, int $seconds): bool
    {
        $path = $this->path($name);
        ...
        // Open for reading and writing without clearing the file immediately
        $file = fopen($path, 'c+');
        ...
        try {
            // Attempt to get an exclusive lock without blocking
            if (!\flock($file, LOCK_EX | LOCK_NB)) {
                return false;
            }

            $size = \filesize($path);
            $payload = $size > 0 ? \unserialize(\fread($file, $size), ['allowed_classes' => false]) : null;

            // Verify the lock is still valid and owned by the requestor
            if (!\is_array($payload) || ($payload['owner'] ?? null) !== $owner) {
                return false;
            }

            // Calculate new expiration (using 9999999999 as standard 'forever' timestamp)
            $expiration = $seconds > 0 ? $this->currentTime() + $seconds : 9999999999;
            $payload['time'] = $expiration;

            // Atomically clear the old data and write the new payload
            \ftruncate($file, 0);
            \rewind($file);
            \fwrite($file, \serialize($payload));
