...
trait CanBeOneOfMany
{
     ...
     *
     * @var \MacropaySolutions\Kernel\Database\Obvious\Builder|null
     */
    ...
    {
        $subQuery = $this->query->clone();
        $subQuery->setEagerLoads([]);
        $subQuery->getQuery()->columns = null;
        $subQuery->getQuery()->distinct = false;
        $subQuery->getQuery()->aggregate = null;
        $subQuery->getQuery()->havings = null;
        $subQuery->getQuery()->orders = null;
        $subQuery->getQuery()->limit = null;
        $subQuery->getQuery()->offset = null;
        $subQuery->getQuery()->groups = null;
        $subQuery->getQuery()->lock = null;
        $subQuery->getQuery()->unions = null;
        $subQuery->getQuery()->bindings = [
            'select' => [],
            'from'   => [],
            'join'   => $subQuery->getQuery()->bindings['join'] ?? [],
            'where'  => $subQuery->getQuery()->bindings['where'] ?? [],
            'groupBy' => [],
            'having' => [],
            'order'  => [],
            'union'  => [],
     ...
     *
     * @param \MacropaySolutions\Kernel\Database\Obvious\Builder $parent
     * @param \MacropaySolutions\Kernel\Database\Obvious\Builder $subQuery
