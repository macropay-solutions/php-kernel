...
class MySqlSchemaState extends SchemaState
{
    ...
    public function dump(Connection $connection, $path)
    {
        $this->executeDumpProcess(
            $this->makeProcess(
                $this->baseDumpCommand() . ' --routines --result-file="${:FRAMEWORK_LOAD_PATH}" --no-data'
            ),
            $this->output,
    ...
    public function load($path)
    {
        $command = 'mysql ' . $this->connectionString() .
            ' --database="${:FRAMEWORK_LOAD_DATABASE}" < "${:FRAMEWORK_LOAD_PATH}"';
    ...
    protected function baseDumpCommand()
    {
        ...
        return $command . ' "${:FRAMEWORK_LOAD_DATABASE}"';
    }
    ...
    {
        $value = ' --user="${:FRAMEWORK_LOAD_USER}" --password="${:FRAMEWORK_LOAD_PASSWORD}"';

        $config = $this->connection->getConfig();

        $value .= $config['unix_socket'] ?? false
            ? ' --socket="${:FRAMEWORK_LOAD_SOCKET}"'
            : ' --host="${:FRAMEWORK_LOAD_HOST}" --port="${:FRAMEWORK_LOAD_PORT}"';

        if (isset($config['options'][\PDO::MYSQL_ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:FRAMEWORK_LOAD_SSL_CA}"';
        }
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            'FRAMEWORK_LOAD_SOCKET' => $config['unix_socket'] ?? '',
            ...
            'FRAMEWORK_LOAD_PASSWORD' => $config['password'] ?? '',
            'FRAMEWORK_LOAD_DATABASE' => $config['database'],
            'FRAMEWORK_LOAD_SSL_CA' => $config['options'][\PDO::MYSQL_ATTR_SSL_CA] ?? '',
        ];
    }
