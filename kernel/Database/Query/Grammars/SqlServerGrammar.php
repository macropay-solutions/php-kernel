...
namespace MacropaySolutions\Kernel\Database\Query\Grammars;

use MacropaySolutions\Kernel\Database\Query\Builder;
use MacropaySolutions\Kernel\Database\Query\JoinLateralClause;
...
class SqlServerGrammar extends Grammar
{
     ...
     *
     * @param \MacropaySolutions\Kernel\Database\Query\Builder $query
     * @param \MacropaySolutions\Kernel\Database\Query\IndexHint $indexHint
     * @return string
     */
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $sql .= 'using (values ' . $parameters . ') ' . $this->wrapTable('framework_source') . ' (' . $columns . ') ';

        $on = collect($uniqueBy)->map(function ($column) use ($query) {
            return $this->wrap('framework_source.' . $column) . ' = ' . $this->wrap($query->from . '.' . $column);
        })->implode(' and ');
        ...
        if ($update) {
            $update = collect($update)->map(function ($value, $key) {
                return is_numeric($key)
                    ? $this->wrap($value) . ' = ' . $this->wrap('framework_source.' . $value)
     ...
     *
     * @param \MacropaySolutions\Kernel\Database\Query\JoinLateralClause $join
     ...
     *
     * @param \MacropaySolutions\Kernel\Contracts\Database\Query\Expression|string $table
     * @return string
     */
