...
use MacropaySolutions\Kernel\Contracts\Validation\ImplicitRule;
...
class Validator implements ValidatorContract
{
     ...
     *
     * @var \MacropaySolutions\Kernel\Contracts\Translation\Translator
     */
     ...
     *
     * @var \MacropaySolutions\Kernel\Validation\PresenceVerifierInterface
     */
     ...
     *
     * @var \MacropaySolutions\Kernel\Support\MessageBag
     */
    protected $messages;
     ...
     *
     * @param \MacropaySolutions\Kernel\Contracts\Translation\Translator $translator
     * @param array $data
     * @param array $rules
    ...
    public function passes()
    {
        ...
        // Here we will spin through all the "after" hooks on this validator and
    ...
    protected function validateAttribute($attribute, $rule)
    {
        ...
        if ($rule instanceof Unique) {
            if (!$this->executeQueryObjectValidation($attribute, $this->getValue($attribute), $rule)) {
                $this->addFailure($attribute, 'unique', [$rule->getTable(), $rule->getColumn()]);
            }
        ...
        if ($rule instanceof Exists) {
            if (!$this->executeQueryObjectValidation($attribute, $this->getValue($attribute), $rule)) {
                $this->addFailure($attribute, 'exists', [$rule->getTable(), $rule->getColumn()]);
            }
        ...
        try {
            ...
            if ($this->messages instanceof MessageBag) {
                ...
                /**
                 * Add implicit Bail on exception
                 */
                $this->failedRules[$this->replacePlaceholderInString($attribute)]['Bail'] = [];
    ...
    protected function replaceEscapedDotInKeyParameters(array $rules): array
    {
     ...
     * @param string $attribute
     * @param mixed $value
     * @param \MacropaySolutions\Kernel\Contracts\Validation\Rule $rule
     * @return void
     */
    ...
    protected function shouldStopValidating($attribute)
    {
        ...
        return isset($this->failedRules[$cleanedAttribute]) && ([] !== \array_intersect(
            $keys = \array_keys($this->failedRules[$cleanedAttribute]),
            [
                'uploaded',
                'Numeric',
                'Array',
                ...
                \MacropaySolutions\Kernel\Validation\Rules\Enum::class,
            ]
        ) || (
            [] !== \array_intersect($keys, $this->implicitRules) &&
    ...
    {
        $this->initialRules = $rules = $this->replaceEscapedDotInKeyParameters($rules);
    ...
    public function sometimes($attribute, $rules, callable $callback)
    {
        ...
        foreach ((array)$attribute as $key) {
            $key = \str_replace('\.', '__dot__' . static::$placeholderHash, $key);
            ...
            foreach ($response->rules as $ruleKey => $ruleValue) {
                if ($callback($payload, $this->dataForSometimesIteration($ruleKey, !str_ends_with($key, '.*')))) {
                    $this->addRules([$ruleKey => $ruleValue], false);
                }
            }
        }
     ...
     * @return \MacropaySolutions\Kernel\Support\Fluent|array|mixed
     */
     ...
     * @return \MacropaySolutions\Kernel\Validation\PresenceVerifierInterface
     *
     ...
     *
     * @param \MacropaySolutions\Kernel\Validation\PresenceVerifierInterface $presenceVerifier
     * @return void
     */
     ...
     *
     * @return \MacropaySolutions\Kernel\Contracts\Translation\Translator
     */
    ...
    /**
     * Execute a unique validation check directly using the object state.
     * @throws \Throwable
     */
    protected function executeQueryObjectValidation(string $attribute, mixed $value, Unique|Exists $rule): bool
    {
        [$connection, $table, $idColumn] = $this->parseTable($rule->getTable());

        $column = $rule->getColumn() === 'NULL' ? $attribute : $rule->getColumn();

        $query = \app('db')->connection($connection)
            ->table($table)
            ->where($column, $value);

        if ($rule instanceof Unique && $ignoreId = $rule->getIgnoreId()) {
            $query->where($idColumn ?? $rule->getIdColumn(), '!=', $ignoreId);
        }

        foreach ($rule->getWheres() as $where) {
        ...
        return $rule instanceof Unique ? !$query->exists() : $query->exists();
    }
}
