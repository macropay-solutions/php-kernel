...
class Batch implements Arrayable, JsonSerializable
{
     ...
     *
     * @var \MacropaySolutions\Kernel\Bus\BatchRepository
     */
    ...
    public function add($jobs)
    {
        ...
        if (\is_array($jobs) && \array_is_list($jobs) && \is_string($jobs[0] ?? null)) {
            $jobs = [$jobs];
        }
    ...
    protected function prepareBatchedChain(array $chain)
    {
        return collect($chain)->map(function ($job) {
            ...
            if (\is_array($job) && \array_is_list($job) && \is_string($job[0] ?? null)) {
                // It looks like ['Class', 'Method'], so it's a Storable Array
                $job = CallQueuedCallable::create($job);
            }
    ...
    protected function invokeHandlerCallback($handler, Batch $batch, ?Throwable $e = null)
    {
        try {
            ...
            return \MacropaySolutions\Kernel\Queue\CallQueuedCallable::invoke(
                [$handler[0], $handler[1]],
                $e !== null ? [
                    'batch' => $batch,
                    Batch::class => $batch,
